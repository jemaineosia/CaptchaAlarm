...
- **Cooldown Period** – 10-second cooldown prevents alarm spam

---

## License Management

AlarmTool uses a per-machine HWID licensing system. Use `LicenseManager.ps1` (located in the project root) to manage tester access.

### Prerequisites

Run commands from the project root (`C:\Development\CaptchaAlarm`).

### Commands

```powershell
# List all current licenses
powershell -ExecutionPolicy Bypass -File ".\LicenseManager.ps1" list

# Activate a new tester (7 days)
powershell -ExecutionPolicy Bypass -File ".\LicenseManager.ps1" add -Hwid "AABBCCDD11223344" -Days 7

# Activate a new tester (30 days)
`powershell -ExecutionPolicy Bypass -File ".\LicenseManager.ps1" add -Hwid "AABBCCDD11223344" -Days 30`

# Extend / reset an existing license from today
powershell -ExecutionPolicy Bypass -File ".\LicenseManager.ps1" extend -Hwid "AABBCCDD11223344" -Days 30

# Revoke access
powershell -ExecutionPolicy Bypass -File ".\LicenseManager.ps1" remove -Hwid "AABBCCDD11223344"
```

### Tester Flow

1. Send `AlarmTool.exe` to the tester
2. Tester runs it → sees **"Activation Required"** screen with their HWID
3. Tester copies HWID and emails it to you (`jemaine.osia@gmail.com`)
4. You run `add -Hwid "..." -Days N`
5. Tester restarts the app → licensed ✓

> **Note:** The HWID is also always visible in the **Statistics** panel inside the app.
