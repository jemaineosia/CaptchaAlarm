    ...
    internal static class Program
    {
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                // Check license — run on thread pool to avoid blocking the STA thread
                var license = Task.Run(() => TrialService.CheckAsync()).GetAwaiter().GetResult();
                AppLogger.Log($"[License] Status={license.Status}, DaysLeft={license.DaysLeft}");

                switch (license.Status)
                {
                    case TrialService.TrialStatus.NotActivated:
                        using (var f = new ActivationRequiredForm(TrialService.GetHwidPublic()))
                            f.ShowDialog();
                        return;

                    case TrialService.TrialStatus.Expired:
                        using (var f = new TrialExpiredForm(TrialService.GetHwidPublic()))
                            f.ShowDialog();
                        return;
                }
